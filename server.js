...
async function init() {
  ...
  if (rows[0].c === 0) {
    ...
    await pool.query(`INSERT INTO store(key,value) VALUES('users',$1) ON CONFLICT(key) DO NOTHING`, [JSON.stringify([{ id:'admin', name:'Administrator', role:'admin', pass:'••••••' }])]);
    console.log('Seeded default admin: admin / admin123');
  }
  console.log('Database ready.');
}
async function getKey(key) { const { rows } = await pool.query('SELECT value FROM store WHERE key=$1', [key]); return rows.length ? rows[0].value : null; }
async function setKey(key, value) { await pool.query(`INSERT INTO store(key,value) VALUES($1,$2) ON CONFLICT(key) DO UPDATE SET value=excluded.value`, [key, JSON.stringify(value)]); }
...
app.put('/api/item/:key', auth, async (req, res) => {
  try {
    const key = req.params.key, item = req.body.item;
    if (!item || !item.id) return res.status(400).json({ error: 'item with id required' });
    const list = (await getKey(key)) || [];
    const i = list.findIndex(x => x && x.id === item.id);
    if (i >= 0) list[i] = item; else list.push(item);
    await setKey(key, list);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: String(e.message) }); }
});

app.delete('/api/item/:key/:id', auth, async (req, res) => {
  try {
    const { key, id } = req.params;
    const list = (await getKey(key)) || [];
    await setKey(key, list.filter(x => !(x && x.id === id)));
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: String(e.message) }); }
});
...
app.get('/', (_req, res) => res.send('Skill Seekers CRM backend (Supabase, v3) is running.'));
